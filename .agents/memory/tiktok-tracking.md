...
- ClickButton events use object form `{contentId, contentName, contentType}`. Tracked: hero CTA (`start-ai-transformation`), explore nav cards (`explore-<id>`), phone/call links (`call-button`), WhatsApp button (`whatsapp-button`). Lead content_name is "AI Consultation Request".
- WhatsApp button lives in the details-page contact section, links to `https://wa.me/601111469065` (Joel's number 011-11469065 → 60 + drop leading 0). There is no other WhatsApp UI; the WhatsApp-named asset files are unrelated media.
